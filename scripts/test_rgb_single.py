...
def get_model(mtype,sigma,ishape,device):
    if mtype == "batched":
        model = nlrn.batched.load_model(sigma,nlrn_pad=True).to(device)
        model.eval()
    elif mtype == "original":
        model = nlrn.refactored.load_model(sigma,"original").to(device)
        model.eval()
    else:
...
# -- create timer --
timer = nlrn.utils.timer.ExpTimer()
...
noisy_fns = nlrn.utils.io.save_burst(noisy/255.,out_dir,"noisy")
deno_fns = nlrn.utils.io.save_burst(deno,out_dir,"deno")
print(timer)
